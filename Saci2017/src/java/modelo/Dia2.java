...
@XmlRootElement
@NamedQueries({
    @NamedQuery(name = "Dia2.findAll", query = "SELECT d FROM Dia2 d")
    , @NamedQuery(name = "Dia2.findById", query = "SELECT d FROM Dia2 d WHERE d.id = :id")
    , @NamedQuery(name = "Dia2.findByPresencaManha", query = "SELECT d FROM Dia2 d WHERE d.presencaManha = :presencaManha")
    , @NamedQuery(name = "Dia2.findByPresencaTarde", query = "SELECT d FROM Dia2 d WHERE d.presencaTarde = :presencaTarde")
    , @NamedQuery(name = "Dia2.findByCoffeeBreak", query = "SELECT d FROM Dia2 d WHERE d.coffeeBreak = :coffeeBreak")})
public class Dia2 implements Serializable {
    ...
    public boolean equals(Object object) {
        ...
        if (!(object instanceof Dia2)) {
            return false;
        }
        Dia2 other = (Dia2) object;
