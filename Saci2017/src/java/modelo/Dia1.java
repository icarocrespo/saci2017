...
@XmlRootElement
@NamedQueries({
    @NamedQuery(name = "Dia1.findAll", query = "SELECT d FROM Dia1 d")
    , @NamedQuery(name = "Dia1.findById", query = "SELECT d FROM Dia1 d WHERE d.id = :id")
    , @NamedQuery(name = "Dia1.findByPresencaManha", query = "SELECT d FROM Dia1 d WHERE d.presencaManha = :presencaManha")
    , @NamedQuery(name = "Dia1.findByPresencaTarde", query = "SELECT d FROM Dia1 d WHERE d.presencaTarde = :presencaTarde")
    , @NamedQuery(name = "Dia1.findByCoffeeBreak", query = "SELECT d FROM Dia1 d WHERE d.coffeeBreak = :coffeeBreak")})
public class Dia1 implements Serializable {
    ...
    public boolean equals(Object object) {
        ...
        if (!(object instanceof Dia1)) {
            return false;
        }
        Dia1 other = (Dia1) object;
